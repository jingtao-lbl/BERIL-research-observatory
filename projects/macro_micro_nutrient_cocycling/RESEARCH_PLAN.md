...
- **v5** (2026-05-13): Soil+plant subset analysis. Repeated entire analysis pipeline filtered to 4,540 soil/rhizosphere and plant-associated species (from `env_species_mapping.csv`). Scripts modified in-place on branch `projects/macro_micro_nutrient_cocycling_v4_soil_plant`. Key changes: stronger P×M phi (0.129 vs 0.110), greater phylogenetic attenuation (41% vs 2%), reduced phenazine operon carriers (27 vs 63), non-significant Wang 2021 phytase×siderophore result. Environmental stratification section removed (not applicable within single-environment subset).
